    ...
    return(
        ...
        {file ? <div className="flex justify-center bg-[#F1F4F9] flex-col items-center">
            <p className="mt-20">
                    ...
                    <Document file={file} className="940:w-[55rem] w-11/12 mb-5 " onLoadSuccess={onDocumentLoadSuccess} onLoadError={()=>console.log('hi')}>
                    {numArray.map((pageNumber,index) => {
                        return(
                            <>
                <Page pageNumber={pageNumber} onRenderSuccess={onPageLoadSuccess} key={index} className="mt-5 page shadow-2xl" width={windowWidth >= 940 ? 880 : (11/12)*windowWidth} /> 
                </>)
                    })}
